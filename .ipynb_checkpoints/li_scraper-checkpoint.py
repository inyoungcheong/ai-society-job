...
class LinkedInGeminiScraper:
    ...
    def create_search_combinations(self) -> List[Dict]:
            """Create minimized search combinations - 키워드 중심"""
            combinations = []
            
            # 핵심 AI & Society 키워드 (확장)
            ai_society_queries = [
                # Core Ethics & Policy
                "AI ethics", "responsible AI", "AI policy", "AI governance", "algorithmic fairness", "AI safety",
                
                # Policy & Law  
                "technology policy", "digital policy", "digital rights", "AI regulation", "data ethics",
                
                # Research & Academic
                "AI research", "computational social science", "AI and society", "digital humanities",
                
                # Industry Applications
                "responsible technology", "AI risk management", "ethical AI consultant", "AI transparency",
                
                # Emerging Areas
                "tech for good", "AI democratization", "inclusive AI", "public interest technology"
            ]
            
            # 지역을 최소화: 3개만 사용
            minimal_locations = [
                "United States",  # 가장 넓은 범위 
                "Remote",         # 원격 근무
                "Europe"          # 유럽 전체
            ]
            
            # 키워드별로 3개 지역 = 20 × 3 = 60개
            for query in ai_society_queries:
                for location in minimal_locations:
                    combinations.append({
                        "keyword": query,
                        "location": location,
                        "experienceLevel": "",  # 모든 레벨
                        "jobType": "full time",
                        "dateSincePosted": "past month",  # 더 넓은 범위
                        "limit": "25",
                        "priority": "high"
                    })
            
            print(f"📊 Minimized to {len(combinations)} combinations")
            print(f"   🔍 {len(ai_society_queries)} keywords × {len(minimal_locations)} locations")
            print(f"   ⏱️ Estimated time: {len(combinations) * 2 / 60:.0f} minutes")
...
def main():
    ...
    scraper = LinkedInGeminiScraper()  # ← 이 부분을 수정 (원래는 LinkedInJobScraper였음)
